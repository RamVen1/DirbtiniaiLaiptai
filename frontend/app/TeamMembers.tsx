  ...
  return (
    <SafeAreaView className="flex-1 bg-background">
      <View className="flex-1">
        ...
        {loading ? (
          <View className="flex-1 justify-center items-center"><ActivityIndicator color={tint} /></View>
        ) : (
            ...
            <View className="bg-card border border-border/20 rounded-3xl p-6">
              ...
              <View className="mt-4 gap-3">
                {members.map((member: any) => (
                  <Pressable key={member.id} onPress={() => handleReviewStats(member)} className="bg-background border border-border/20 rounded-2xl p-4 active:opacity-80">
